 ...
 * @author ASUS
 */
@WebServlet(name = "simpan_perubahanPadaProduk", urlPatterns = {"/simpan_perubahanPadaProduk"})
public class simpan_perubahanPadaProduk extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        OBJ_produk produk_ = new OBJ_produk();
        String idProduk = request.getParameter("idProduk");
        int stok_brg = Integer.parseInt(request.getParameter("stokProduk"));
        Double harga_brg = Double.parseDouble(request.getParameter("hargaProduk"));
        produk_.setId_produk(idProduk);
        produk_.setNama_produk(request.getParameter("namaProduk"));
        produk_.setJenis_produk(request.getParameter("jenisProduk"));
        produk_.setKeterangan_produk(request.getParameter("keteranganProduk"));
        produk_.setStock_produk(stok_brg);
        produk_.setHarga_produk(harga_brg);
        
        try{
            Class.forName("com.mysql.jdbc.Driver");
            Connection conn = db_connection.connect_to_Db();
            
            PreparedStatement prpEdit = conn.prepareStatement("UPDATE tabel_produk SET nama_produk=?, stok_produk=?, keterangan_produk=?, jenis_produk=?, harga_produk=? WHERE product_id=?");
            prpEdit.setString(1, produk_.getNama_produk());
            prpEdit.setInt(2, produk_.getStock_produk());
            prpEdit.setString(3, produk_.getKeterangan_produk());
            prpEdit.setString(4, produk_.getJenis_produk());
            prpEdit.setDouble(5, produk_.getHarga_produk());
            prpEdit.setString(6, produk_.getId_produk());
            prpEdit.executeUpdate();
            conn.close();
            
            try {
                out.println("<script type=\"text/javascript\">");
                out.println("alert('Berhasil edit detail barang');");
            ...
            Logger.getLogger(simpan_perubahanPadaProduk.class.getName()).log(Level.SEVERE, null, ex);
