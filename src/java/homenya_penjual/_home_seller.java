        ...
        if (cookies != null) {
            ...
            if (levelAkun.equals("seller")) {
                try {
                    ...
                    PreparedStatement psjmlhreq = conn.prepareStatement("SELECT COUNT(*) AS jumlahReq FROM tabel_kotak_masuk WHERE request_ke=? AND status_request='barumasuk'");
                    psjmlhreq.setString(1, idAkun);
                    ResultSet rs3 = psjmlhreq.executeQuery();
                    int jumlah_req = 0;

                    while (rs3.next()) {
                        jumlah_req = rs3.getInt("jumlahReq");
                    }

                    PreparedStatement psjmlhproduk = conn.prepareStatement("SELECT COUNT(*) AS jumlahbarang FROM tabel_produk WHERE inserted_by=?");
                    psjmlhproduk.setString(1, idAkun);
                    ...
                    try {
                        out.println("<!DOCTYPE html>\n"
                                ...
                                + "				<div class=\"col-xs-6 col-md-3 col-lg-3 no-padding\">\n"
                                + "					<div class=\"panel panel-orange panel-widget border-right\">\n"
                                + "						<div class=\"row no-padding\"><em class=\"fa fa-xl fa-exclamation-triangle\"></em>\n"
                                + "							<div class=\"large\">" + jumlah_req + "</div>\n"
                                + "							<div class=\"text-muted\">Request Belum Direspon</div>\n"
                                + "						</div>\n"
                                + "					</div>\n"
                                + "				</div>\n"
                                + "				<div class=\"col-xs-6 col-md-3 col-lg-3 no-padding\">\n"
