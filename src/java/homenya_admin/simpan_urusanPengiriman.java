...
@WebServlet(name = "simpan_urusanPengiriman", urlPatterns = {"/simpan_urusanPengiriman"})
public class simpan_urusanPengiriman extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        Part filePart;
        filePart = request.getPart("logo_shipping");
        inputStream = filePart.getInputStream();
        PrintWriter out = response.getWriter();
        Random rnd = new Random();
        int number = rnd.nextInt(999999);

        String id_kurirSTR = String.format("%06d", number);
        int idnyaKurir = Integer.parseInt(id_kurirSTR);

        try {
            Class.forName("com.mysql.jdbc.Driver");
            Connection conn = db_connection.connect_to_Db();

            OBJ_kurir kurir = new OBJ_kurir();
            kurir.setId_kurir(idnyaKurir);
            kurir.setNama_kurir(request.getParameter("nama_jasaKirim"));
            kurir.setNotel_kurir(request.getParameter("noHp_jasaKirim"));
            kurir.setAlamatkantor_kurir(request.getParameter("alamat_jasakirim"));

            PreparedStatement PSaddkurir = conn.prepareStatement("INSERT INTO tabel_kurir VALUES (?,?,?,?,?)");
            PSaddkurir.setInt(1, kurir.getId_kurir());
            PSaddkurir.setString(2, kurir.getNama_kurir());
            PSaddkurir.setString(3, kurir.getNotel_kurir());
            PSaddkurir.setString(4, kurir.getAlamatkantor_kurir());
            PSaddkurir.setBlob(5, inputStream);
            PSaddkurir.executeUpdate();
            conn.close();

            try {
                out.println("<script type=\"text/javascript\">");
                out.println("alert('Berhasil menambahkan layanan kirim');");
