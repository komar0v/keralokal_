...
import homenya_admin.view_urusanPengiriman;
 ...
 * @author ASUS
 */
@WebServlet(name = "seller_accountSetting", urlPatterns = {"/seller_accountSetting"})
public class seller_accountSetting extends HttpServlet {
        ...
        if (cookies != null) {
            ...
            if (levelAkun.equals("seller")) {
                try {
                    ...
                    PreparedStatement ps = conn.prepareStatement("SELECT * FROM tabel_toko WHERE id_toko=?");
                    String idnyatoko = request.getParameter("idToko_");
                    ps.setString(1, idnyatoko);

                    ResultSet rs = ps.executeQuery();
                    if (rs.next()) {
                        toko_.setNama_toko(rs.getString("nama_toko"));
                    }
                    ...
                    Logger.getLogger(seller_accountSetting.class.getName()).log(Level.SEVERE, null, ex);
