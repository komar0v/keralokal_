...
@WebServlet(name = "simpan_urusanPembayaran", urlPatterns = {"/simpan_urusanPembayaran"})
public class simpan_urusanPembayaran extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        Part filePart;
        filePart = request.getPart("logo_payment");
        inputStream = filePart.getInputStream();
        PrintWriter out = response.getWriter();
        Random rnd = new Random();
        int number = rnd.nextInt(999999);

        String id_pymtSTR = String.format("%06d", number);
        int paymentmethod_id = Integer.parseInt(id_pymtSTR);

        try {
            Class.forName("com.mysql.jdbc.Driver");
            Connection conn = db_connection.connect_to_Db();

            OBJ_metodepembayaran payment = new OBJ_metodepembayaran();
            payment.setId_paymentMethod(paymentmethod_id);
            payment.setNama_paymentMethod(request.getParameter("nama_penyediajasabayar"));
            payment.setNotel_paymentMethod(request.getParameter("noHp_penyediajasabayar"));
            payment.setAlamatkantor_paymentMethod(request.getParameter("alamat_penyediajasabayar"));

            PreparedStatement PSaddpayment = conn.prepareStatement("INSERT INTO tabel_metodebayar VALUES (?,?,?,?,?)");
            PSaddpayment.setInt(1, payment.getId_paymentMethod());
            PSaddpayment.setString(2, payment.getNama_paymentMethod());
            PSaddpayment.setString(3, payment.getNotel_paymentMethod());
            PSaddpayment.setString(4, payment.getAlamatkantor_paymentMethod());
            PSaddpayment.setBlob(5, inputStream);
            PSaddpayment.executeUpdate();
            conn.close();

            try {
                out.println("<script type=\"text/javascript\">");
                out.println("alert('Berhasil menambahkan metode pembayaran');");
